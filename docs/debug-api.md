...
- `metadata`：元数据查询；`resolve` / `refresh`：首次取链或过期刷新。
- `cache_check`：缓存直链 Range 校验；`redirect`：交付 302 给客户端。
...
例如 `cache_check status=0 error=dns` 后出现 `redirect status=302`，表示服务器校验网络不确定，仍按现有语义交由客户端直连，并不表示服务端成功请求了音频。`403/404/410` 可能表示缓存链接失效；刷新结果和后续阶段有助区分取链与音频请求问题。`200/206` 只证明服务端拿到了这些响应头，可能仍是错误内容、编解码不支持、客户端网络/CORS/混合内容或签名绑定问题。

建议排查顺序：查看模式和音源健康 → 用户在自己的音流客户端重试指定歌曲 → 查结构化阶段/分类 → 必要时显式授权 probe 交叉检查。服务端探测无法观察音流客户端实际网络。不要索取用户全部歌单、原始日志或永久凭据。`force_redirect` 继续禁止客户端 `proxy=1` 覆盖，不会因诊断失败自动代理；独立 probe 只读取服务端响应头，不向客户端转发音频。
...
覆盖认证隔离、管理员/CSRF/TTL、撤销/到期/创建者失效的在途取消、数量/限频/并发、恶意错误脱敏、DNS绑定与重定向/私网/TLS、无播放持久化副作用、实际 h_media 事件及强制302、前端一次显示/复制失败/撤销/清理。浏览器 fixture 的音频在 localhost，主动探测预期返回 `blocked_target`，不会为测试放松生产 SSRF规则。
