...
- 直链缓存在独立的 `data/url-cache.db` 中保存，按歌曲和请求音质区分，重启后继续复用；读取或重启不会延长过期时间。「永久」不按时间过期，仍受链接失效、音源变更和最多 2000 条的最近最少使用淘汰约束。缓存数据库故障时退化为内存缓存，下次启动重建后恢复持久化。
- 修改缓存档位只清空对应缓存；修改服务器名称等无关设置不清缓存。音源新增、启停、重载、删除、更新脚本或调整优先级后，内存与持久直链缓存同时失效；启动时也会核对音源和代理配置。已开始传输的音频不受影响。
- **302 播放与下载**在复用缓存前，以 `GET` 和 `Range: bytes=0-0` 轻量校验响应头，最多等待 3 秒并立即关闭响应体，不读取或转发整首音频；同一缓存版本的并发校验会合并。`200/206` 继续复用，`403/404/410` 时重新取链一次并返回新链接；刷新失败返回错误。网络错误、超时、`429/5xx` 等不确定结果保留缓存并继续返回 302，让客户端直连尝试。新解析的链接不额外校验，校验失败不会自动切换代理。
...
- 两次取链共用累计 45 秒预算，缓存校验有独立的 3 秒上限，不限制整首音频传输时长。普通 **302 模式**（未请求代理时）与**强制 302 模式**均使用上述缓存校验，音频仍由客户端直接下载；服务端无法收到音流直连失败的通知，校验也不能代替客户端网络或解码能力检查。
