...
(() => {
  ...
  var hasHeader = (headers, name) => Object.keys(headers).some((k) => k.toLowerCase() === name);
  ...
  // shims/crypto.js
  var toBuf = (d, enc) => Buffer.isBuffer(d) ? d : Buffer.from(String(d), enc || "utf8");
  var Hash = class {
    constructor(alg, key) {
      this.alg = alg;
      this.key = key;
    ...
    update(data, enc) {
      this.chunks.push(toBuf(data, enc));
      return this;
    }
    digest(enc) {
  ...
  var createHmac = (alg, key) => new Hash(String(alg).toLowerCase(), toBuf(key));
  var Cipher = class {
  ...
  var createCipheriv = (mode, key, iv) => new Cipher(false, mode, key, iv);
  var createDecipheriv = (mode, key, iv) => new Cipher(true, mode, key, iv);
  ...
  // shims/zlib.js
  var toBuf2 = (d) => Buffer.isBuffer(d) ? d : Buffer.from(d);
  var mk = (op) => (data, options, cb) => {
    if (typeof options === "function") cb = options;
    let result, error = null;
    try {
      result = __host.zlib(op, toBuf2(data));
    } catch (e) {
      error = e instanceof Error ? e : new Error(String(e));
    }
    Promise.resolve().then(() => cb(error, result));
  };
  var mkSync = (op) => (data) => __host.zlib(op, toBuf2(data));
  ...
  var zlib_default = { inflate, deflate, inflateRaw, deflateRaw, gunzip, gzip, unzip, inflateSync, deflateSync, inflateRawSync, deflateRawSync, gunzipSync, gzipSync, unzipSync };
  ...
  if (typeof g.document !== "object") g.document = { cookie: "", createElement() {
    return {};
  ...
  g.crypto = Object.assign({}, crypto_default, { subtle: void 0, webcrypto: void 0 });
  ...
  var nativeRequire = typeof g.require === "function" ? g.require : null;
  var modules = { crypto: crypto_default, zlib: zlib_default, needle: needle_default, buffer: { Buffer: import_buffer.Buffer } };
  g.require = (name) => {
  ...
  var lx = {
    ...
    request(url, options, callback) {
      ...
      const opts = {
        headers,
        response_timeout: typeof timeout === "number" && timeout > 0 ? Math.min(timeout, 6e4) : 6e4
      };
      let data;
      if (body) {
        data = body;
      ...
      let req = needle_default.request(method, url, data, opts, (err, resp, body2) => {
        try {
          if (err) {
            callback.call(this, err, null, null);
          } else {
            body2 = resp.body = resp.raw.toString();
            try {
    ...
    utils: {
      crypto: {
        aesEncrypt(buffer, mode, key, iv) {
          const cipher = crypto_default.createCipheriv(mode, key, iv);
          return Buffer.concat([cipher.update(buffer), cipher.final()]);
        },
        rsaEncrypt(buffer, key) {
          buffer = Buffer.concat([Buffer.alloc(128 - buffer.length), buffer]);
          return crypto_default.publicEncrypt({ key, padding: crypto_default.constants.RSA_NO_PADDING }, buffer);
        },
        randomBytes(size) {
          return crypto_default.randomBytes(size);
        },
        md5(str) {
          return crypto_default.createHash("md5").update(str).digest("hex");
        }
      },
      buffer: {
        from(...args) {
      ...
      zlib: {
        inflate(buf) {
          return new Promise((resolve, reject) => {
            zlib_default.inflate(buf, (err, data) => {
        ...
        deflate(data) {
          return new Promise((resolve, reject) => {
            zlib_default.deflate(data, (err, buf) => {
              if (err) reject(new Error(err.message));
              else resolve(buf);
            });
          });
